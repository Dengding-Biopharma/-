...
def find_significant(filename,mode,keywords):
    ...
    if x_index and y_index:
        ...
        print('One or two group has no value! keywords: {}, mode: {}'.format(keywords,mode))

if __name__ == '__main__':
    modes = ['BOTH','POS','NEG']
    keywords_list = [['XYCH_WX_','XYCH_WXPB_'],['GYCH_WX_','GYCH_WXPB_'],['GWBZ_WX_','GWBZ_WXPB_'],['GHH_WX_','GHH_WXPB_'],['GCH_WX_','GCH_WXPB_'],['WX_','WXPB_']]
    for mode in modes:
        if mode == "BOTH":
